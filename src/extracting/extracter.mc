...
let extract : ExtractingOptions -> DocTree -> ObjectTree =
    use TokenReader in use BreakerChooser in use Objects in
    lam opt. lam tree.
    ...
    lam tree. lam namespace. lam commentBuffer. lam sourceCodeBuilder. lam inStdlib. lam utestCount. lam depth.
        ...
        case DocTreeNode { children = children, token = token, state = state } then
            ...
            let buildDoc : [String] -> String = lam commentBuffer.
                ...
                match res with "" then objDefaultDoc else res in
            ...
            let process : State -> [DocTree] -> String -> String -> String -> Object -> Int -> ExtractRecOutput =
